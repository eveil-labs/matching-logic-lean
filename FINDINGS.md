...
and immediately after Theorem 3.8:
...
("In [3], it yields Theorem 16, the weak completeness statement"). In the
technical report [5] the corresponding numbers are Theorem 83 (strong) and
Theorem 16 (weak).
...
Corollary 16, Remark 17, and Section 6. **(S) is discharged rather than
assumed** — every rule of Figure 2 is proved sound — so Corollary 15 is
available with (L) as the only remaining hypothesis.
...
Proposition 30 is in progress. (L) is not attempted; we estimate discharging it
at several thousand lines, since it is the canonical-model construction that the
rest of the paper is designed to avoid.

---

## 8. Questions where we had to choose, and would rather you decided

Mechanizing forces a reading of things prose leaves open. In each case below we
picked one, checked it does not break your lemmas, and in some cases checked
that the alternatives do. But the choice is yours, and if any of these is not
what you meant we would like to correct it.
...
`CORRESPONDENCE.md` is generated by joining `gate/certified.txt` with the
kernel's own `#print axioms` output. **A result it does not list is a result we
are not claiming.** The only `sorry`s in the repository are the five deliberate
stubs in `variants/` — the refuted side of each prove-or-refute pair.

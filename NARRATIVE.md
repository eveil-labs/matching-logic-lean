...
points of increasing ambition: (i) Lemmas 9 and 11 alone; (ii) Corollary 15 with
(L) strong local completeness and (S) soundness assumed; (iii) discharge (L) as
well, "yielding a self-contained machine-checked global completeness theorem".
...
countably infinite element-variable type — which is the qualification that
matters and is stated wherever the claim is made.

## 2. One representation choice, made early, that decided everything after

Patterns are **raw named syntax**. `Pattern` is an inductive type over named
variables, and it is *not* quotiented by α-equivalence. Substitution is a
function on that type; capture-avoidance is a side condition, not a convention.
...
For entry points (i) and (ii) the choice was free. Lemmas 9 and 11 and Theorem
...
Entry point (iii) is where that bill came due. A canonical-model construction
...
The obvious hypothesis, and the one we started from, was that this was waste:
that a better definition, or the quotient, would make it go away.
...
`FreshWitnessed` — demands a witness avoiding **all** variables of the body,
bound ones included. Four questions, each settled by machine-checked proof:

**Do the two conditions actually differ?** Yes.
`witnessed_freshWitnessed_of_isMCS_refuted` exhibits a maximal locally
consistent set that is witnessed and not fresh-witnessed. Empty signature,
carrier `Bool`, a valuation naming `true` by the variable `0` and everything
else by `false`, and the complete theory of the point `true`. It is witnessed
because that valuation is surjective — every satisfied existential has *some*
name for its witness. It is not fresh-witnessed because `∃0. var 0` belongs to
it while `0` is the only name for `true`, and `0` occurs in the body. Maximality
is discharged, not assumed.

**Is that an artifact of refusing the quotient?** No.
...
**Then what does repair it?** The supply.
`freshWitnessed_of_witnessed_of_supply`: if every existential has *infinitely
many* usable Henkin names, fresh-witnessedness follows, because the body has
...
already all of `ℕ` and cannot be extended, the hypothesis has to appear
explicitly, as `InfiniteFreshVariableSupply`. It is a translation of the
extension step, not a correction of it.

**Is the hypothesis load-bearing?** Yes, and this is the sharp end.
`locConsistent_extend_freshWitnessed_isMCS_unrestricted_refuted`: drop it and the
statement is false. Feed the extension the witnessed-but-not-fresh MCS from the
first result. It is already maximal, so any locally consistent extension equals
it, so it would have to be fresh-witnessed itself — which is refuted.
...
the question we were least likely to get wrong — a bad proof does not compile
and is fixed within the minute.

Everything else was got wrong at least once. Across the development and nine
rounds of adversarial review, **twenty-six real defects** were recorded, and the
kernel caught **none** of them. They were wrong statements, wrong prose, wrong
claims about what had been proved, and — most often — wrong *instruments*:
gates that could not fail, gates that verified themselves, gates that reported
success having compared nothing. Round eight found an `axiom` declaration that
passed every check in the repository. Round nine found that the repair to round
eight had, in three separate places, gone on to identify our own code *by its
name* — and was walked past by writing the same declaration outside the
namespace, or marking it `private`.
...
of the most important checks are **not ours at all**:
`lake exe axiom-audit` from leanprover-community, and `lake exe mk_all --check`,
which comes with Mathlib. `lake env leanchecker`, the kernel replay built into
the toolchain, replays the whole library independently. Preferring maintained
tools to hand-rolled ones was not modesty; the hand-rolled ones had the worse
defect record.
...
Twice while establishing the results in §3, we generalized past what a probe had
actually checked.
...
α's reach. That reasoning assumed α renames only bound names. True of syntactic
α; false of the coarser proof-theoretic relation this development uses in its
place, under which the pattern is equivalent to a vacuously quantified one. An
independent lane refuted it, and found the countermodel that does work.

Both errors were caught before anything shipped, and both have the same shape.
It seemed worth writing down, in a document whose subject is a distinction that
took four independent attempts to state correctly.
